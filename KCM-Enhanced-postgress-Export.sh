...
# --------------------------------------------------------------------------------
# Keeper PAM Export Tool (PostgreSQL Version)
# ----------------------------------------
# Description:
#   This script exports Apache Guacamole connection data from PostgreSQL databases
#   into Keeper PAM-ready formats for import via Keeper Commander CLI. It safely 
#   patches the docker-compose file to expose database ports if needed, and provides 
#   JSON export files that can be imported using the Keeper Commander tool.
#
# Usage Examples:
#   ./keeper-pam-export-postgres.sh                # Interactive mode
#   ./keeper-pam-export-postgres.sh --export       # CLI export with defaults
#
# Flags:
...
# --------------------------------------------------------------------------------
show_version() {
    echo "Keeper PAM Export Tool (PostgreSQL Version) v$VERSION"
    echo "Optimized for Keeper PAM import compatibility with PostgreSQL databases"
    exit 0
}
...
check_dependencies() {
    ...
    # Check if PostgreSQL connector is installed 
    if ! python3 -c "import psycopg2" 2>/dev/null; then
        log_warn "PostgreSQL Connector (psycopg2) for Python not found. Will attempt to install it."
        log_info "Installing PostgreSQL Connector for Python..."
        pip install psycopg2-binary --quiet || {
            log_error "Failed to install psycopg2-binary. Please install it manually."
            exit 1
        }
        log_success "PostgreSQL Connector installed successfully."
    else
        log_debug "PostgreSQL Connector for Python is already installed."
    fi

    # Optional command checks with helpful instructions
    if ! command -v psql &>/dev/null; then
        log_warn "psql command not found. PostgreSQL connectivity checks will be basic."
        log_debug "Install with: apt-get install postgresql-client or yum install postgresql"
    fi
...
inspect_compose_file() {
    log_info "Inspecting docker-compose.yml for PostgreSQL service and port configuration..."
    ...
    # Enhanced compose file parsing: Focus on PostgreSQL service and port detection
    db_info=$(python3 - <<EOF
import yaml, json
try:
    with open("$COMPOSE_FILE", "r") as f:
        ...
        # First priority: Look specifically for Guacamole PostgreSQL DB
        for service_name, service_config in data['services'].items():
            # Look for environment variables that indicate a Guacamole DB
            if 'environment' in service_config:
                env = service_config['environment']
                env_dict = {}
                
                # Convert environment to dictionary for easier checking
                ...
                # Check for Guacamole-specific environment variables
                has_guacamole_vars = any(key in env_dict for key in 
                                        ['GUACAMOLE_DATABASE', 'GUACAMOLE_USER', 'GUACAMOLE_USERNAME', 
                                         'GUACAMOLE_PASSWORD', 'GUACAMOLE_ADMIN_PASSWORD'])
                
                # Also check if image name suggests PostgreSQL
                image = service_config.get('image', '').lower()
                is_postgres = 'postgres' in image or 'postgresql' in image
                
                if has_guacamole_vars and is_postgres:
                    db_service = service_name
                    break

        # Second priority: Look for any PostgreSQL service if no Guacamole DB found
        if not db_service:
            for service_name, service_config in data['services'].items():
                image = service_config.get('image', '').lower()
                # Identify PostgreSQL service by image name
                if 'postgres' in image or 'postgresql' in image:
                    db_service = service_name
                    break

        # If we found a PostgreSQL service, check port configuration
        if db_service:
            service_config = data['services'][db_service]
            ...
            if 'ports' in service_config:
                ...
                for port in ports:
                    if isinstance(port, dict) and 'published' in port and 'target' in port:
                        # "5432:5432" format as dictionary
                        port_mapping = f"{port['published']}:{port['target']}"
                        ports_exposed = True
                        break
                    elif isinstance(port, str):
                        # Handle string format like "5432:5432" or "5332:5432"
                        if ':' in port:
                            host_port, container_port = port.split(':')
                            port_mapping = port
                            ports_exposed = True
                            break
                        # Handle direct port exposure like "5432" 
                        else:
                            port_mapping = port
                            ports_exposed = True
                            break
            ...
            if 'environment' in service_config:
                ...
                for key, value in env_dict.items():
                    if isinstance(key, str) and ('PORT' in key.upper() or 'DATABASE_PORT' in key.upper()) and value:
                        port_mapping = f"{value}:{value}" if not ports_exposed else port_mapping
                        break

        # Fallback to 'db' if no PostgreSQL service found by image
        if not db_service:
            for service_name, service_config in data['services'].items():
                # Check for generic 'db' service
                if service_name.lower() in ['db', 'database', 'postgres', 'postgresql']:
                    image = service_config.get('image', '').lower()
                    # Verify it's likely a PostgreSQL service
                    if 'postgres' in image or 'postgresql' in image or 'database' in service_name.lower():
                        db_service = service_name
                        break
                # Also check for guacamole-db-postgres or similar names
                elif 'guacamole' in service_name.lower() and ('db' in service_name.lower() or 'database' in service_name.lower()):
                    image = service_config.get('image', '').lower()
                    if 'postgres' in image or 'postgresql' in image:
                        db_service = service_name
                        break
        ...
        if db_service:
            ...
            result = {
                "status": "error",
                "message": "No PostgreSQL service found in docker-compose.yml"
            }
...
EOF
    ...
    default_port=$(echo "$db_info" | jq -r '.default_port')

    log_info "Found PostgreSQL service: $DB_SERVICE"

    if [[ "$ports_exposed" == "true" && -n "$port_mapping" ]]; then
        log_success "PostgreSQL port is exposed: $port_mapping"

        # Extract host port from mapping (e.g., "5432:5432" → "5432")
        if [[ "$port_mapping" == *":"* ]]; then
            host_port=$(echo "$port_mapping" | cut -d':' -f1)
            container_port=$(echo "$port_mapping" | cut -d':' -f2)
            DB_PORT="$host_port"
            log_info "PostgreSQL port mapping: Host port $host_port → Container port $container_port"
        else
            DB_PORT=$port_mapping
            log_info "PostgreSQL port directly exposed as: $DB_PORT"
        fi

        log_info "Using port $DB_PORT for PostgreSQL connection"
    else
        log_warn "PostgreSQL port is NOT exposed in docker-compose.yml"
...
patch_compose_file() {
    ...
    log_info "Setting up port mapping for PostgreSQL database (default internal port: $container_port)"

    read -p "Enter the host port to expose for PostgreSQL (default: $DB_PORT): " custom_port
...
EOF
    ...
    if [[ "$status" == "error" ]]; then
        ...
        log_success "Compose file patched to expose port ${custom_port} → ${container_port}"
        log_info "PostgreSQL will be accessible on host port $custom_port"
    fi
...
extract_db_credentials() {
    ...
    if [[ -z "$DB_SERVICE" ]]; then
        log_error "PostgreSQL service name not available. Run inspect_compose_file first."
        exit 1
    fi

    # Enhanced credential extraction specifically focused on Guacamole PostgreSQL credentials
...
prompt_manual_credentials() {
    log_info "Please enter PostgreSQL connection details manually:"
    ...
    read -p "PostgreSQL username ${default_user}: " input_user
    read -sp "PostgreSQL password: " input_pass
    echo ""
    read -p "PostgreSQL database name ${default_db}: " input_db
    ...
    if [[ -z "$DB_USER" || -z "$DB_PASS" || -z "$DB_NAME" ]]; then
        log_error "All PostgreSQL connection fields are required"
        exit 1
    fi

    log_info "Manual PostgreSQL connection details accepted"
}

# ================================================================================
# SECTION 13: DATABASE CONNECTION VERIFICATION
# --------------------------------------------------------------------------------
# Purpose: Tests PostgreSQL connection with retry and fallback mechanisms
# --------------------------------------------------------------------------------
verify_db_connection() {
    ...
    if test_postgres_connection "$DB_HOST" "$DB_PORT" "$DB_USER" "$DB_PASS" "$DB_NAME"; then
    ...
    if [[ "$DB_HOST" == "localhost" && -n "$DB_SERVICE" ]]; then
        ...
        if test_postgres_connection "$DB_SERVICE" "$DB_PORT" "$DB_USER" "$DB_PASS" "$DB_NAME"; then
            CONNECTION_VERIFIED=true
            DB_HOST="$DB_SERVICE"
    ...
    if [[ -n "$ADMIN_PASS" ]]; then
        ...
        if test_postgres_connection "$DB_HOST" "$DB_PORT" "$DB_USER" "$ADMIN_PASS" "$DB_NAME"; then
            CONNECTION_VERIFIED=true
            DB_PASS="$ADMIN_PASS"
    ...
    if test_postgres_connection "$DB_HOST" "$DB_PORT" "postgres" "$DB_PASS" "$DB_NAME"; then
        CONNECTION_VERIFIED=true
        DB_USER="postgres"
    ...
    for alt_db in "guacamole" "guacamole_db" "postgres" "public"; do
        if [[ "$alt_db" != "$DB_NAME" ]]; then
            ...
            if test_postgres_connection "$DB_HOST" "$DB_PORT" "$DB_USER" "$DB_PASS" "$alt_db"; then
                CONNECTION_VERIFIED=true
                DB_NAME="$alt_db"
                log_success "PostgreSQL connection successful with database: $DB_NAME"
                return 0
            fi
        fi
    done
...
export_connection_data() {
    log_info "Exporting connection data from Guacamole PostgreSQL database to Keeper PAM format..."

    if [[ "$CONNECTION_VERIFIED" != "true" ]]; then
        log_error "PostgreSQL connection not verified. Run verify_db_connection first."
        exit 1
    fi
    ...
    # Execute the PostgreSQL-specific data export using Python for better error handling and formatting
...
try:
    ...
    cursor.execute("SELECT to_regclass('public.guacamole_connection')")
    if not cursor.fetchone()[0]:
        print(json.dumps({
            "status": "error",
            "message": "Required Guacamole tables not found in PostgreSQL database"
        }))
        sys.exit(0)

    # Get connection data with parameters (PostgreSQL version)
    cursor.execute("""
        SELECT
            c.connection_id,
            c.connection_name,
    ...
    for conn_id, conn in connections.items():
        ...
        if hostname and ':' in hostname:
            ...
            if not port:
                ...
                print(f"DEBUG: Extracted port {port} from hostname string for connection {conn['name']}")
            ...
            if url:
                try:
                    ...
                    if ':' in hostname:
                        ...
                        if not port:
                            port = hostname_parts[1]
                            print(f"DEBUG: Extracted port {port} from URL hostname for connection {conn['name']}")
                except:
                    pass
...
Machine Records,${machine_records_count}"
        ...
        # Perform automatic import using Keeper Commander
...
# --------------------------------------------------------------------------------
main() {
    log_info "Starting Keeper PAM Export Tool (PostgreSQL Version) v$VERSION"
